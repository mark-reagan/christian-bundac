...
export default function RequesterDashboard() {
	...
	const equipmentRequests = useApiRequest(
		(signal) => equipmentRequestsApi.list({ page: 1 }, signal),
		[],
	);
	const supplyRequests = useApiRequest(
		(signal) =>
			isFaculty
				? supplyRequestsApi.list({ page: 1 }, signal)
	...
	return (
		<div className="space-y-6">
			<div
				className={`grid grid-cols-1 gap-4 ${isFaculty ? 'sm:grid-cols-2' : ''}`}
			>
				<QuickLink
					to="/equipment"
					icon="box"
					label="Browse Equipment"
					description="Find and request equipment for your event."
				/>
				{isFaculty && (
					<QuickLink
						to="/supplies"
						...
						description="Request consumable supplies."
					/>
				)}
			</div>
			...
			<div className="card">
				<h2 className="mb-3 text-base font-semibold text-slate-900">
					Your recent equipment requests
				</h2>
