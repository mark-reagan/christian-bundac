...
The Docker entrypoint runs `php artisan migrate --force` before Apache starts, so pending migrations are applied automatically on Render deployments, including the Free plan where Shell and pre-deploy commands may be unavailable. A migration failure stops the container from serving requests; check the deploy logs and database connectivity if startup fails. Do not run production seeders without reviewing their default credentials. Render's local filesystem is ephemeral; use a managed database and durable object storage if the app later needs persistent uploads.
